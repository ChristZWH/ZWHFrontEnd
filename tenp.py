...
# 不同色块的hsv范围
color_range = {
    'green': [(54 , 83 , 105 ),( 81 , 151 , 180 )],
    'orange': [( 15 , 176 , 123 ),( 18 , 283 , 177 )]
}
...
def Box_Back(n):
    for i in range (0,n):
            base_action.action("Boxback1")
            time.sleep(0.5)
def Box_Back1(n):
    for i in range (0,n):
            base_action.action("Boxback2")
            time.sleep(0.5)
def Back2(n):
    for i in range (0,n):
            base_action.action("Backrun1")
            time.sleep(0.5)
def Back1(n):
    for i in range (0,n):
            base_action.action("Backrun1")
...
def box_go(n):
    for i in range (0,n):
        base_action.action("Boxforward7-10")
        time.sleep(0.5)
def box_go1(n):    #抱着箱子前进一步
    for i in range (0,n):
        base_action.action("无法失去的抱住")
        time.sleep(1)
def box_go2(n):     #抱着箱子前进两步
    for i in range (0,n):
        print("报箱前进两步")
        base_action.action("狠狠的向前走两步1")
        time.sleep(1)
...
def box_under():
    base_action.action("摔倒1")  # 抱着箱子前进一步
...
def BoxL_move1 (n):           #左侧移  
    for i in  range (0,n):
        base_action.action ("Boxleft7-10")
        time.sleep(1)
def BoxL_move2 (n):           #左侧移
    for i in  range (0,n):
        base_action.action('Box-move-left1')
        time.sleep(1)
def BoxL_move3 (n):           #左侧移
    for i in  range (0,n):
        base_action.action('Left_Move')
        time.sleep(1)
def R_move1 (n):           #右侧移  
    for i in  range (0,n):
...
def BoxR_move2 (n):           #右侧移 
    for i in  range (0,n):
        base_action.action('Box-move-right1')
        time.sleep(1)
def BoxR_move3 (n):           #右侧移 
    for i in  range (0,n):
        print("通道报箱子右移")
        base_action.action('通道完美右移')
        time.sleep(1)
...
def BoxR_turn1 (n):           #右转
    for i in  range (0,n):
        print("报箱子右转函数1")
        base_action.action("BoxturnR7-10")
        time.sleep(1)
    ...
    if chest_circle_x is None :
        ...
        if chest_circle_x < 275:
            ...
            if chest_circle_y < 300:  
                ...
                base_action.action("前进一小步")
                base_action.action("前进一小步")
                base_action.action("最完美的抱抱箱子_4")
...
def Box_Go_new(n):           #前进一步 
    for i in  range (0,n):
        print("抱箱子前进1步函数NEW）")
        base_action.action("慢抱抱1")
        time.sleep(1)

def Box_Go_new3(n):           #前进三步 
    for i in  range (0,n):
        print("抱箱子前进3步函数NEW）")
        base_action.action("抱抱向前3步2")
        time.sleep(1)

def Box_LMove_new(n):           #左移 
    for i in  range (0,n):
        print("抱箱子左移函数NEW")
        base_action.action("左移一步1")
        time.sleep(1)

def Box_RMove_new(n):           #右移 
    for i in  range (0,n):
        print("抱箱子右移函数NEW")
        base_action.action("右移一步1")
        time.sleep(1)

def Box_LTurn_new(n):           #左转 
    for i in  range (0,n):
        print("抱箱子左转函数NEW")
        base_action.action("左转1")
        time.sleep(1)

def Box_RTurn_new(n):           #右转 
    for i in  range (0,n):
        print("抱箱子右转函数NEW")
        base_action.action("右转1")
        time.sleep(1)

def Box_Go_new_2(n):           #前进一步 
    for i in  range (0,n):
        print("抱箱子前进1步函数NEW）")
        base_action.action("最好的慢走1")
        time.sleep(1)

def Box_RTurn_new_2 (n):           #右转
    for i in  range (0,n):
        print("报箱子右转函数2")
        base_action.action("微调右转1")
        time.sleep(1)
        ...
        if ID == 0:  # 搬箱子
            ...
            if len(marker) == 0:
                ...
                if step == 1:
                    if marker[0] == 1: 
                        level = "start_moving"
                        result = turn_to_tag(robot_tag_x, robot_tag_y, tag_yaw, 0.08, -0.03, 2)
                        if result == True:
                            ID += 2
                            print('一号码')
                            for i in range(7):
                        ...
                        if ID == 3:
                            result = turn_to_tag(robot_tag_x, robot_tag_y, tag_yaw, 0.10 , 0.06 , 2)
                            if result == True:
                                ID += 1
                        ...
                        if ID == 4:
                            result = turn_to_tag(robot_tag_x, robot_tag_y, tag_yaw, 0.12 , 0.05 , 2)
                            if result == True:
